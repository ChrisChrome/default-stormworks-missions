...
function onTick(delta_worldtime)
	...
	elseif(g_savedata.tutorial_stage == STAGE_OUTSIDE_HOUSE) then
		...
		for player_index, player_object in pairs(players) do
			...
			local distSQ = ((-43 - x)^2) + ((24.7 - y)^2) + ((3 - z)^2)
			if(distSQ < 20)
			then
	...
	elseif(g_savedata.tutorial_stage == STAGE_AT_HILL) then
		...
		for player_index, player_object in pairs(players) do
			...
			local distSQ = ((-55 - x)^2) + ((17.7 - y)^2) + ((31 - z)^2)
			if(distSQ < 10)
			then
	...
	elseif(g_savedata.tutorial_stage == STAGE_AT_STAIRS) then
		if(g_savedata["starterBoatSpawned"] == false) then
			...
			server.setCurrency(server.getCurrency() - 19800, server.getResearchPoints())
		...
		for player_index, player_object in pairs(players) do
			...
			local distSQ = ((-28 - x)^2) + ((6 - y)^2) + ((16 - z)^2)
			if(distSQ < 10) then
				...
				if (survivor["rescued"] == false) then
					...
					if survivor["healed"] ~= (c.hp >= 100) then
						...
						if(c.hp < 100) then
							...
							server.setPopup(-1, g_savedata[survivor["name"]], "survivor" .. the_id, true, "Rescue this survivor to the hospital located on this island", 0, 1.5, 0, 0, 0, the_id)
						end
					end
				end
			end
...
function UpdateUI()
	if(g_savedata.tutorial_stage == STAGE_OUTSIDE_HOUSE) then
		server.setPopup(-1, g_savedata["UIID"], "obj", true, "Walk to the Hill", -43, 24.7, 3, 0)
	elseif(g_savedata.tutorial_stage == STAGE_AT_HILL) then
		server.setPopup(-1, g_savedata["UIID"],"obj", true, "Walk to the Stairs", -55, 17.7, 31, 0)
	elseif(g_savedata.tutorial_stage == STAGE_AT_STAIRS) then
		server.setPopup(-1, g_savedata["UIID"],"obj", true, "Walk to the Dock", -28, 6, 16, 0)
	...
	elseif(g_savedata.tutorial_stage == STAGE_LEFT_DOCK) then
		server.setPopup(-1, g_savedata["UIID"],"obj", true, "Navigate to the source of the distress call", g_savedata["destX"], g_savedata["destY"] + 20, g_savedata["destZ"], 0)
	elseif(g_savedata.tutorial_stage == STAGE_ARRIVED_AT_BOAT) then
		for the_id, v2 in pairs(g_savedata["mission"].objects) do
